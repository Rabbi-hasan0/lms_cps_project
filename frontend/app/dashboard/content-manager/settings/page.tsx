...
export default function AdminSettingsPage() {
    ...
    return (
        ...
        {activeTab === 'platform' && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6 cursor-not-allowed select-none">
            <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
                ...
                <span className="px-2.5 py-1 text-[10px] font-semibold bg-slate-800 text-slate-400 border border-slate-700/80 rounded-lg">
                Read Only
                </span>
            </div>

            {/* pointer-events-none দিয়ে ভেতরের সব ক্লিক/টাইপ নিষ্ক্রিয় ও অপাসিটি কমানো হয়েছে */}
            <form onSubmit={(e) => e.preventDefault()} className="space-y-5 pointer-events-none opacity-60">
                ...
                <label className="flex items-center justify-between p-3.5 bg-slate-950 border border-slate-800 rounded-xl">
                    <div>
                ...
                disabled
                type="button"
                className="px-5 py-2.5 bg-slate-800 border border-slate-700 text-slate-500 text-xs font-semibold rounded-xl flex items-center gap-2 shadow transition"
                >
                <Save className="w-3.5 h-3.5" />
                <span>Save System Settings (Disabled)</span>
                </button>
            </form>
            </div>
        )}
